...
namespace TRESORERIE.Controllers
{
    public class ProjetController : Controller
    {
        ...
        public ActionResult Form(string Mode, string Code)
        {
            if (Filter())
            {
                ...
                if (Mode == "Edit")
                {
                    ...
                    decimal TTC=(decimal)(Element.MONTANT_HT + ((Element.MONTANT_HT * Element.TVA) / 100));
                    ViewBag.TTC = TTC.ToString("F3");
                }
                ViewBag.Mode = Mode;
                ViewBag.Code = Code;
                return View(Element);
        ...
        public ActionResult SendForm(string Mode, string Code)
        {
            if (Filter())
            {
                ...
                if (Mode == "Create")
                {
                    ...
                    NewElement.MONTANT_HT = decimal.Parse(MONTANT_HT, NumberStyles.Number, CultureInfo.InvariantCulture);
                    NewElement.TVA = int.Parse(TVA);
                    NewElement.GARANTIE = decimal.Parse(GARANTIE, NumberStyles.Number, CultureInfo.InvariantCulture);
                ...
                if (Mode == "Edit")
                {
                    ...
                    SelectedElement.MONTANT_HT = decimal.Parse(MONTANT_HT, NumberStyles.Number, CultureInfo.InvariantCulture);
                    SelectedElement.TVA = int.Parse(TVA);
                    SelectedElement.GARANTIE = decimal.Parse(GARANTIE, NumberStyles.Number, CultureInfo.InvariantCulture);
        ...
        {
            int CodeInt = int.Parse(Code);
            BD.ACHATS_PREVISIONNELS.Where(p => p.PROJETS.ID == CodeInt).ToList().ForEach(p => BD.ACHATS_PREVISIONNELS.Remove(p));
            BD.BUDGETS.Where(p => p.PROJETS.ID == CodeInt).ToList().ForEach(p => BD.BUDGETS.Remove(p));
            BD.BUDGETS.Where(p => p.PROJETS.ID == CodeInt).ToList().ForEach(p => BD.BUDGETS.Remove(p));
            BD.BUDGETS.Where(p => p.PROJETS.ID == CodeInt).ToList().ForEach(p => BD.BUDGETS.Remove(p));
            BD.BUDGETS.Where(p => p.PROJETS.ID == CodeInt).ToList().ForEach(p => BD.BUDGETS.Remove(p));
            //ACHATS_PREVISIONNELS
            //BUDGETS
            //DEPENSES
            //FACTURATIONS
            //FACTURATIONS_PREVISIONNELS
            PROJETS SelectedElement = BD.PROJETS.Find(CodeInt);
